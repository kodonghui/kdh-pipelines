...
파이프라인 시작 후, Step 0 완료 후, 첫 Phase/Stage 진입 전에 실행.
...
3. 현재 작업과 매칭:
   - planning 모드 → pipeline: "planning" 또는 "all"
   - sprint 모드 → pipeline: "dev" 또는 "all"
   - scope: 현재 Sprint/Story/Stage와 관련된 것만

4. 매칭된 plan 본문 읽기 (Read tool)

5. plan 맥락을 보유하고 실행 시작:
   - plan은 "맥락 제공자" — SKILL.md의 절차를 override하지 않음
