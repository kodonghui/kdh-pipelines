...
41. **Team cleanup + escalation.** 작업이 끝난 팀/agent 를 유휴 상태로 방치하지 않는다.
    - 팀 작업 완료 시 team lead 는 종료 요청 또는 세션 정리 신호를 남긴다.
    - 정리 요청이 반복 무응답이면 새 팀 생성 전에 기존 세션 상태를 확인하고, 살아 있는 유휴 프로세스가 있으면 종료한다.
    - dev 위임 후 무응답이 지속되면 conductor/main 이 같은 task 를 직접 회수해 처리한다.
    - 이 규칙은 fresh team 원칙을 지키기 위한 운영 규칙이며, 기존 v2 roster(winston/dev/john/sally)를 되돌리지 않는다.
