...
| 3 — Validate | PRD 검증 + `kdh-handoff-verify` (Sprint N+1+, PHX-005) | Grade C (Writer Solo) | ❌ 자동 PASS |
...
3. UI Design System (project-context.yaml `ui.components` 기반):
   - IF `ui.components == "shadcn"`: → MCP 체크 불필요. ✅ 자동 통과.
   - IF `ui.components == "subframe"`: → ToolSearch로 subframe 도구 검색 → 미인증이면 🚩 BLOCK
   - IF 비어있으면: → ⚠️ WARNING. UI 컴포넌트 라이브러리 미설정.
...
## Step 0-pre: Planning Active 상태 설정 (v10.6)

**Planning Pipeline 시작 시 반드시 실행. 이 단계 없이 planning-artifacts/ 수정 불가 (Hook 차단).**

```
1. pipeline-state.yaml에 planning_active: true 설정:
   - sed 또는 직접 Edit으로 planning_active: true 추가/변경
   - 이미 true면 그대로 유지

2. 파이프라인 종료 시 (정상/비정상 모두):
   - planning_active: false 설정
   - Stop hook에서도 false 복구 (비정상 종료 대비)

3. 긴급 우회: PLANNING_GUARD_BYPASS=1 환경변수 (CEO 전용)
```

★ 이 단계를 건너뛰면 planning-artifact-guard.sh Hook이 모든 Edit/Write를 차단합니다.
...
2. status: active 필터링
   ...
   ★ superseded plan은 reopen record (status:active + reopens_plan_id) 경로로만 살릴 수 있음
...
## Step -1 (Topic 4 Sprint N+1+ ACTIVATE): Phase Handoff Bridge

**Status:** Reader compatibility landed Sprint N. Writer (kdh-phase-bridge) activates Sprint N+1. In Sprint N, this step **reads** prior phase-handoff if present and **tolerates absence** silently.

Sprint N+1부터 Stage 0 (Brief) 진입 **이전**에 실행.

```
1. 이전 phase handoff 파일 존재 확인
   _bmad-output/retro/phase-handoff/{N-1}.yaml

2. 존재하면 → kdh-phase-bridge (Sprint N+1 신규 스킬) invoke
   - 입력: {N-1}.yaml + Topic 2 LESSONS.jsonl + _index.yaml active
   - 출력: Context Block (헤더 "CRITICAL: CONTEXT CARRIED OVER. DO NOT RE-LITIGATE.")
   - 부수효과: carry-forward와 매칭되는 stale_plan_ids 자동 close (quorum 조건 충족 시)

3. Context Block을 Stage 0 Brief 작성 프롬프트의 맨 앞에 prepend

4. 부재하면 → handoff-buffer.tmp fallback (PHX-015)
   - parent_commit_sha 유효성 검증
   - divergent → reject as Untrusted/Divergent + log

5. 둘 다 없으면 → Sprint N 호환 경로 (Step 0 바로 진입, Brief 무컨텍스트)
```

Related EARS: PHX-004, PHX-006, PHX-007, PHX-015, PHX-020, PHX-021.
