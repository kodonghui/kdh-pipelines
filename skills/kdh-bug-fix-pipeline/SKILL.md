...
### 구조 유지 (절대)
- **Phase 1 SCAN → 2 FIX → 3 SWEEP → 4 DEPLOY 유지**
- **3사 sweep** (OpenAI + Gemini + Claude) 유지
- **TeamCreate + origin 분류 + party-log 2명** 유지

### 속도 최적화
1. **Sweep 캐싱 (변경 페이지만 re-sweep)**: bug-fix-state.yaml에 `last_sweep_pages` 기록. 새 커밋의 diff에서 영향 받는 페이지만 re-sweep. Sprint End에는 항상 full-sweep.
2. **Origin 자동 분류**: Codex가 root_cause 분석할 때 origin(code/design/requirements/test)도 함께 제안. BUG-020 같은 origin 미기재 재발 방지.
3. **Outer loop SCAN 재사용**: outer_loop 2회차+에서 **새 커밋이 없으면 SCAN 생략**하고 FIX만 반복.
4. **Codex/Gemini 백그라운드**: sweep + review 시 `codex-review.sh v2` + Bash `run_in_background: true` 사용.
...
Step 3e: Cross-Model 일괄 리뷰 (Codex + Gemini)
  전체 변경사항을 Codex+Gemini 병렬 리뷰:
  
  git diff main...HEAD > /tmp/bugfix-diff.patch
  bash ~/.claude/scripts/codex-review.sh /tmp/bugfix-diff.patch \
    "이 코드 변경을 리뷰해라. 버그 수정 diff다.
  각 변경이:
  1. 근본 원인을 고쳤는가?
  2. 부작용은 없는가?
  3. 회귀 테스트가 충분한가?
  ...
  → 하나라도 PASS → Phase 4로 (두 결과 합산)
  → 둘 다 FAIL → 이슈 수정 → Step 3e 재실행 (max 1 retry)
  → 둘 다 실행 불가 (인증/타임아웃) → CEO 보고, 자동 스킵 금지
...
24. **Cross-Model Sweep 필수.** Phase 1 SCAN에서 GPT-5.4 + Gemini 3.1 Pro 두 모델로 browser-use sweep 실행. 두 모델이 동시에 보고한 버그는 오탐(false positive)으로 분류 금지 — 반드시 실제 버그로 등록하고 조사. 한 모델만 보고한 건은 코드 확인 후 판단.
25. **Gemini API 키.** `.env`에 `GEMINI_API_KEY` 추가. browser-use에서 `ChatGoogle(model='gemini-3.1-pro-preview', api_key=...)` 사용.
...
### 주의사항
- `browser_use.llm.openai.chat.ChatOpenAI` 사용 (GPT-5.4 모델)
...
- **Phase End 자동 연동 (v11.0, 수정 2026-04-16):** Phase 마지막 Sprint의 Sprint End에서만 필수 호출. 중간 Sprint는 tsc + bun test만. CEO 결정: sweep 시간 과다 → Phase End 1회로 축소.
