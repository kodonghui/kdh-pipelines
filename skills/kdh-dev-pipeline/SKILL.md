...
## Mode C: Parallel Story Dev → kdh-dev-pipeline-mode-c

본 wrapper 호출자: `/kdh-dev-pipeline parallel <id1> <id2> [<id3>]` (max 3 workers).
실제 흐름 정의 = sub-skill `kdh-dev-pipeline-mode-c` (body-migrated-v1, 분리 일자 2026-04-26).
Worktree rule + Contract & Wiring rule + D3 Codex 전담 룰 모두 sub-skill 본문 참조.

호출 후 자동 `/kdh-integration batch` hand-off. wrapper 가 통합 verdict 발행.

---

## Mode D: Swarm Auto-Epic → kdh-swarm

본 wrapper 호출자: `/kdh-dev-pipeline swarm <epic-id>`.
실제 흐름 정의 = 별 skill `kdh-swarm` (Mode D: Swarm Auto-Epic Worker Loop).
TaskCreate / TaskList / TaskUpdate dependency 자동 해석 + 3 worker self-organizing flow + shutdown→sequential merge→deploy 모두 `kdh-swarm` 본문 참조.

Contract & Wiring 규칙 (shared/src/contracts/ 직렬화, wiring story blockedBy 표기) 동일하게 `kdh-swarm` 적용.
...
## Integration Review → kdh-integration

본 wrapper 호출자: `/kdh-integration batch` (Batch 3 stories 후) / `/kdh-integration sprint` (Sprint 후).
실제 흐름 정의 = 별 skill `kdh-integration` (Level 2 Batch + Level 3 Sprint 통합 리뷰).

핵심 hook (orchestration 수준만 wrapper 가 알면 됨):
- 스토리 레벨 cross-model verification = sub-skill `kdh-dev-pipeline-codex` 가 codex-review.sh 호출
- 배치/스프린트 레벨 cross-model verification = `kdh-integration` 안에서 동일 codex-review.sh 호출
- 중복 실행 금지 (`kdh-integration` 단독 책임)
- 판정 매트릭스 (둘 다 PASS = 진행 / 둘 다 FAIL = 수정 + PASS 까지 반복 / 하나 FAIL = CEO 판단) = `kdh-integration` 본문 참조

본 wrapper 단계 = Phase D 완료 → Level 2 자동 hand-off → 모든 batch 완료 후 Level 3 hand-off → E2E 진행.
...
| sub-skill | role | wrapper 본문 (분리 후) | status |
|---|---|---|---|
| `kdh-dev-pipeline-phase-a` | Story Create (winston + qa_reviewer party-mode) | redirect stub only | ✅ body-migrated-v1 |
| `kdh-dev-pipeline-phase-b` | Implementation (Codex CLI 전담, D3) | redirect stub only | ✅ body-migrated-v1 |
| `kdh-dev-pipeline-phase-d` | Test + QA (qa_reviewer + Cross-model) | redirect stub only | ✅ body-migrated-v1 |
| `kdh-dev-pipeline-codex` | 매 story 1회 Codex review (Plan v4 백그라운드 병렬) | redirect stub only | ✅ body-migrated-v1 |
| `kdh-dev-pipeline-mode-c` | Parallel Story Dev (max 3 worktree) | redirect stub only (line 598) | ✅ body-migrated-v1 (2026-04-26 신설) |
| `kdh-swarm` (별 skill) | Mode D: Swarm Auto-Epic Worker Loop | redirect stub only (line 608) | ✅ pre-existing |
| `kdh-integration` (별 skill) | Level 2 Batch + Level 3 Sprint 통합 리뷰 | redirect stub only (line 763) | ✅ pre-existing |
